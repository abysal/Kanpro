...
public class UpdateSubmission extends AppCompatActivity implements OnPageChangeListener, OnLoadCompleteListener,
        OnPageErrorListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_update_submission);
        actionBar=getSupportActionBar();
        actionBar.setDisplayHomeAsUpEnabled(true);
        actionBar.setTitle("Update Submission");
        ...
        current_file=(TextView)findViewById(R.id.current_file);

        Bundle bundle=getIntent().getExtras();
        if (bundle!=null){
            et_assignment_title.setText(bundle.getString("assignment_title"));
            et_assignment_links.setText(bundle.getString("assignment_links"));
            current_file.setText("Current File:"+bundle.getString("assignment_file_user"));
            submission_id=bundle.getString("id");
    ...
    private void uploadFile() {
        if (pdfPath == null) {
            ...
            createInstance();
            Call<SubmissionResponse> call = submissionAPI.updateFile(submission);
            StrictMode();
            try{
                ...
                Toast.makeText(UpdateSubmission.this, "Error", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
        }
    }
    private void updateSubmission(){
        ...
        if (pdfPath!=null){
            ...
            Call<SubmissionResponse> submissionResponseCall=submissionAPI.updateSubmission(submission_id,submission);
            submissionResponseCall.enqueue(new Callback<SubmissionResponse>() {
                ...
                public void onResponse(Call<SubmissionResponse> call, Response<SubmissionResponse> response) {
                    if(!response.isSuccessful()){
                        Toast.makeText(UpdateSubmission.this, "Code"+response.code(), Toast.LENGTH_SHORT).show();
                        pd.dismiss();
                        return;
                    }
                    else if (response.body().getSuc_message()!=null){
                        Toast.makeText(UpdateSubmission.this, response.body().getSuc_message(), Toast.LENGTH_SHORT).show();
                        Intent intent=new Intent(UpdateSubmission.this, Dashboard.class);
                ...
                public void onFailure(Call<SubmissionResponse> call, Throwable t) {
                    Toast.makeText(UpdateSubmission.this, "Error:"+t.getMessage(), Toast.LENGTH_SHORT).show();
                    pd.dismiss();
                }
            });
        }
